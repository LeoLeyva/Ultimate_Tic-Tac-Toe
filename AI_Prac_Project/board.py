...
class Board:
    ...
    alpha_to_num = {"A": 0, "B": 1, "C": 2, "D": 3,
                    "E": 4, "F": 5, "G": 6, "H": 7, "I": 8}
    moves_left = {"0": 9, "1": 9, "2": 9, "3": 9,
                  "4": 9, "5": 9, "6": 9, "7": 9, "8": 9}
    coords_to_board = {(0, 0): "0", (1, 0): "1", (2, 0): "2", (0, 1): "3",
    ...
    def update_board(self, coords, player, prev_coords):
        try:
            (x1, y1) = (self.alpha_to_num[coords[1].capitalize(
            )], self.alpha_to_num[coords[0].capitalize()])
        except:
            raise ex.CoordException("Invalid Coordinates! Try again!")

        try:
            (x2, y2) = (self.alpha_to_num[prev_coords[1].capitalize(
            )], self.alpha_to_num[prev_coords[0].capitalize()])
        except:
