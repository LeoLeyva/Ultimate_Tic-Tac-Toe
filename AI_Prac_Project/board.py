...
class Board:
  ...
  def __init__(self):
    ...
    for l in range(0,9):
      ...
      for k in range(0, 9):
        mid_row.append(" ")
      self.matrix.append(mid_row)

  def print_board(self):
    tb_row = ["    A ", " B ", " C ", "    D ", " E ", " F ", "    G ", " H ", " I"]
  ...
  def ttt_row_check(self, c, r):
    o_streak = 0
    x_streak = 0

    for row in range(r,r+3):
      for col in range(c,c+3):
        cur_char = self.matrix[row][col]
  ...
  def ttt_diag_check(self,r,c):
    d1 = self.matrix[r][c]
    d2 = self.matrix[r+1][c+1]
    d3 = self.matrix[r+2][c+2]

    d4 = self.matrix[r][c+2]
    d5 = self.matrix[r+1][c+1]
    d6 = self.matrix[r+2][c]
  ...
  def ttt_check(self,coords):
    ...
    r = self.ttt_row_check(x, y)
    c = self.ttt_col_check(y,x)
    d = self.ttt_diag_check(y, x)
  ...
  def boards_row_check(self,start, end):
    o_streak = 0
    x_streak = 0
  ...
  def boards_cols_check(self, start):
    o_streak = 0
    x_streak = 0
  ...
  def victory_check(self):
    ...
    r1 = self.boards_row_check(0,3)
    r2 = self.boards_row_check(3,6)
    r3 = self.boards_row_check(6,9)
    ...
    c1 = self.boards_cols_check(0)
    c2 = self.boards_cols_check(1)
    c3 = self.boards_cols_check(2)
